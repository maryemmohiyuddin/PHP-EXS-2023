...
if (isset($_SESSION["user_id"]) && $_SESSION["type"] == "admin") {
    ...
    $topicQuery = "SELECT id, class, subject, topic_name FROM topics";
    $topicResult = $conn->query($topicQuery);

    if (!$topicResult) {
        echo "Error fetching topics: " . $conn->error;
        exit();
    }
    ?>
    ...
    <body>
        ...
        <div class="content">
            <div class="container mt-5">
                ...
                <!-- Display topics in a Bootstrap table -->
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        while ($topicRow = $topicResult->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td>{$topicRow['id']}</td>";
                            echo "<td>{$topicRow['class']}</td>";
                            echo "<td>{$topicRow['subject']}</td>";
                            echo "<td>{$topicRow['topic_name']}</td>";
                            echo "<td><a href='view_quizzes.php?topic_id={$topicRow['id']}'>View Attempted Quizzes</a></td>";
                            echo "</tr>";
                        }
                        ?>
                    </tbody>
